...
@section('content')
    ...
    <div id="set-price-modal"
        ...
        <div class="modal-box bg-white rounded-xl shadow-lg w-full max-w-md p-0 relative" onclick="event.stopPropagation()">
            <div class="flex items-center justify-between py-3 px-4 border-b border-gray-200">
                <h2 class="h6 text-gray-800 font-semibold">Set Warehouse Product Price</h2>
                ...
                <form method="POST" action="" id="set-price-form">
                    ...
                    <div class="form-group mb-4">
                        <label class="block mb-2 text-sm font-medium text-gray-700">
                            Warehouse Product Price <span class="text-red-500">*</span>
                        </label>
                        <div class="relative">
    ...
    <main class="dashboard-screen-bg relative">
        ...
        <section class="dashboard-content py-4 md:py-8 flex-1">
            <div class="container-fluid">
                ...
                <form method="GET" action="{{ route('warehouse.productManagementList') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <a href="{{ route('warehouse.productManagementList') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </form>
                        ...
                        <table class="w-full whitespace-nowrap text-sm text-left text-gray-600">
                            <thead class="text-gray-900 font-semibold uppercase border-b border-gray-200">
                                <tr>
                                    ...
                                    {{-- <th class="px-6 py-3">Warehouse Price</th> --}}
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($products as $product)
                                       <tr class="hover:bg-gray-50">
                                        ...
                                        @if($product->warehouse_price)
                                         <td class="px-6 py-3">
                                            <span class="">${{($product->warehouse_price) }}</span>
                                        </td>
                                        @else
                                         <td class="px-6 py-3">
                                            <span class="">-</span>
                                        ...
                                        {{-- <td class="px-6 py-3">
                                            @if($product->warehouse_price)
                                                <span class="text-blue-600 font-semibold">₹{{ number_format($product->warehouse_price, 2) }}</span>
                                            @else
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <button
                                                onclick='openPriceModal({{ $product->id }}, "{{ addslashes($product->name) }}", "{{ $product->sku }}", {{ $product->min_price }}, {{ $product->max_price }}, {{ $product->warehouse_price ?? 0 }})'
    ...
    <script>
         ...
         */
        function openPriceModal(productId, productName, productSku, minPrice, maxPrice, warehousePrice) {
            const modal = document.getElementById('set-price-modal');
            const form = document.getElementById('set-price-form');
            const priceInput = document.getElementById('warehouse-price-input');
            ...
            const updateUrlBase = "{{ route('warehouse.productUpdatePrice', ['product' => 'PLACEHOLDER']) }}";
            ...
            @if (session('edit_product') && $errors->any())
                priceInput.value = "{{ old('warehouse_price', session('edit_product_price')) }}";
            @else
                priceInput.value = warehousePrice > 0 ? warehousePrice.toFixed(2) : '';
