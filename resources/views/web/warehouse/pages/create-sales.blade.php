        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                <div class="flex flex-wrap gap-[15px] md:gap-[30px]">
                            ...
                            @forelse($products as $product)
                                <div class="white-box text-center flex flex-col gap-[15px] md:gap-[25px] product-card"
                                    ...
                                    <button class="btn btn-outline btn-pink w-full add-product-btn"
                                        ...
                                        data-product-price="{{ $product->warehouse_price ?? 0 }}">
                                        Add
                                    </button>
                                </div>
                            @empty
                    ...
                    <form id="sales-form" action="{{ route('warehouse.storeSales') }}" method="POST">
                        @csrf
                        <div class="max-w-[648px] w-full white-box sticky top-[110px] h-[fit-content]">
                            ...
                            {{-- Display validation errors for sales form (excluding customer form errors) --}}
                            ...
                            <div class="mb-[15px] form-group">
                                ...
                                <input type="text" name="remark" placeholder="Enter remark" class="form-control" value="{{ old('remark') }}">
                            </div>
...
<script>
    ...
    // Restore order state from old input if validation fails
    // function restoreOrderState() {
    //     const oldOrderItems = @json(old('order_items') ?? '{}');
    //     const oldCustomerId = @json(old('customer_id') ?? '');
    //     const oldPaymentMethod = @json(old('payment_method') ?? 'Cash');

    //     // Restore order items
    //     if (oldOrderItems && typeof oldOrderItems === 'object' && Object.keys(oldOrderItems).length > 0) {
    ...
    // ✅ Safely parse old order items (handle string or object)
    try {
    ...
    // ✅ Restore order items if available
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // IMPORTANT: Restore order state first before initializing buttons
        restoreOrderState();
    ...
    {{-- Customer Modal - Only opens when customer form has validation errors --}}
    <dialog id="add-customer-modal" class="modal" @if ($errors->has('phone_number') || $errors->has('name')) open @endif>
        <div class="modal-box p-0 relative">
            ...
            <div class="modal-content p-4">
                <form method="POST" action="{{ route('warehouse.customerStore') }}">
                    @csrf
