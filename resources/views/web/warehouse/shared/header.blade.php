@php
    ...
    // Define menu arrays for active states
    $stockItem = [
        'recieve-stock',
        'recieve-show',
        'transfer-to-outlets',
        ...
        'outlet-stock-request-detail',
    ];
    $stockItems = in_array($currentSegment, $stockItem) ? 'active show' : '';
    
    $cashHandling = ['sales-orders', 'create-sales', 'customer', 'sales-details','product-management','waybill'];
    $saleOrder = in_array($currentSegment, $cashHandling) ? 'active show' : '';
    ...
    $cashHandlings = in_array($currentSegment, $cash) ? 'active show' : '';

    // Initialize all user-related variables
    $user = null;
    $role = '';
    $warehouses = null;
    $outlets = null;
    ...
    // Set values based on authentication guard
    if (Auth::guard('warehouse')->check()) {
        $user = Auth::guard('warehouse')->user();
        $role = 'Warehouse Manager';
        $warehouses = $user->warehouse;
        $unreadCount = $warehouses ? $warehouses->unreadNotifications()->count() : 0;
        $totalNotifications = $warehouses ? $warehouses->notifications()->count() : 0;
    ...
    <div class="container-fluid">
        <div class="drawer">
            ...
            <div class="navbar p-0 bg-white flex items-center gap-[40px] flex-wrap drawer-content">
                <a class="flex items-center" href="{{ route('warehouse.dashboard') }}">
                ...
                <div class="head-right flex-1 hidden lg:flex justify-between gap-[15px] flex-wrap">
                    <!-- Nav links (desktop) -->
                    <ul class="hidden md:flex items-center gap-[5px] menu-list">
                        <li class="dropdown border-0 nav-item {{ request()->is('warehouse/dashboard*') ? 'active' : '' }}">
                        ...
                        @if ($user && $user->hasPermissionTo('stockOperations', 'warehouse'))
                            <li class="dropdown border-0 nav-item stock-transfer-dropdown {{ $stockItems }}">
                                ...
                                <ul tabindex="0"
                                    ...
                                    <li>
                                        <a href="{{ route('warehouse.recieve-stock') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" />
                                            Receive Stock
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.transferoutlets') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" />
                                            Transfer to Outlets
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.return-list') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Returns
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.outlet-levels') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Outlet Levels
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.warehouse-levels') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Warehouse Levels
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.outletStockRequest') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Outlet Stock Request
                                        </a>
                                    </li>
                                </ul>
                            </li>
                        ...
                        @if ($user && $user->hasPermissionTo('sales&Orders', 'warehouse'))
                            <li class="dropdown border-0 nav-item sales-handling-dropdown {{ $saleOrder }}">
                                <a class="flex border-0 focus:outline-none outline-none items-center gap-[10px] px-[15px] py-[10px] rounded-[8px] body-14 semibold"
                                    tabindex="0" role="button">
                                    <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                ...
                                <ul tabindex="0"
                                    class="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm dropmenu">
                                    <li>
                                        <a href="{{ route('warehouse.salesOrders') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" />
                                            Sales & Orders
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.customerList') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" />
                                            Customers
                                        </a>
                                    </li>
                                    <li>
                                        <a href="{{ route('warehouse.productManagementList') }}" class="body-14 medium">
                                            <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" />
                        ...
                        <li class="{{ request()->is('warehouse/downPayment*') ? 'active' : '' }}">
                            <a href="{{ route('warehouse.downPayment') }}"
                                class="flex items-center gap-[10px] px-[15px] py-[10px] rounded-[8px] body-14 semibold">
                                <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                        ...
                        <li class="dropdown border-0 nav-item cash-handling-dropdown {{ $cashHandlings }}">
                            <a class="flex border-0 focus:outline-none outline-none items-center gap-[10px] px-[15px] py-[10px] rounded-[8px] body-14 semibold"
                                tabindex="0" role="button">
                                <svg width="20" height="21" viewBox="0 0 20 21" fill="none"
                                    xmlns="http://www.w3.org/2000/svg">
                            ...
                            <ul tabindex="0"
                                class="dropdown-content menu bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm dropmenu">
                                <li><a href="{{ route('warehouse.daily-sales-summary') }}" class="body-14 medium">
                                    <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Daily Sales Summary</a>
                                </li>
                                <li><a href="{{ route('warehouse.debtCollections') }}" class="body-14 medium">
                                    <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Debt Collections</a>
                                </li>
                                <li><a href="{{ route('warehouse.recordExpenses') }}" class="body-14 medium">
                                    <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Record Expenses</a>
                                </li>
                                <li><a href="{{ route('warehouse.cashRemittance') }}" class="body-14 medium">
                                    <img src="{{ asset('web/images/arrow01.svg') }}" alt="arrow" /> Cash Remittance</a>
                                </li>
                            </ul>
                        </li>
                    </ul>
                        ...
                        <div class="notification-box relative">
                            ...
                            <div id="notificationDropdown" class="notification-dropdown hidden">
                                ...
                                <div class="notification-scroll" id="notificationsList">
                                    @php
                                        $unreadNotifications = $warehouses
                                            ? $warehouses->unreadNotifications()->latest()->take(5)->get()
                                            : collect();
                                    @endphp
                                ...
                                @if ($totalNotifications > 0)
                                    <div class="p-3 border-t text-center bg-white">
                                        <a href="{{ route('warehouse.notifications.all') }}"
                        ...
                        <div class="dropdown">
                            <div tabindex="0" role="button" class="flex items-center gap-[15px]">
                                <figure>
                                    ...
                                    @else
                                        <div class="h-[40px] w-[40px] rounded-full bg-gray-200 flex items-center justify-center figure">
                                ...
                                <figcaption>
                                    @if ($user)
                                        <h5 class="text-sm semibold text-gry-800">
                                            {{ $warehouses ? ucfirst($warehouses->name) : '' }}</h5>
                            ...
                            <ul tabindex="0"
                                ...
                                <li><a href="{{ route('warehouse.myAccount') }}">Edit Profile</a></li>
                                <li><a href="{{ route('warehouse.showChangePasswordForm') }}">Change Password</a></li>
...
<script>
    ...
    function markAsRead(notificationId, redirectUrl = null) {
        fetch(`/warehouse/notifications/${notificationId}/mark-as-read`, {
                method: 'POST',
                headers: {
    ...
    function updateBadgeCount() {
        fetch('/warehouse/notifications/unread-count', {
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
                }
            })
            ...
            .then(data => {
                ...
                if (data.count > 0) {
                    if (badge) {
                        ...
                        newBadge.className = 'absolute -top-1 -right-1 bg-[#ec188b] text-white text-[12px] font-semibold rounded-full w-[18px] h-[18px] flex items-center justify-center';
