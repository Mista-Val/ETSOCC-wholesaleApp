...
@section('content')
    ...
    <main class="dashboard-screen-bg relative">
            ...
            <div class="container-fluid">
                <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                    ...
                    <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                        ...
                        <span class="text-gry-300">/</span>
                        <span class="body-14 text-gry-800">Request from Outlets</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                {{-- <form method="GET" action="{{ route('outlet.outlet-transfer-request') }}" class="search-record-box flex items-center justify-between flex-wrap gap-[15px] mb-[15px]">
   ...
   <input type="date" name="date" value="{{ request('date') }}"
                    ...
                    <a href="{{ route('outlet.outlet-transfer-request') }}" class="btn btn-secondary">Clear Filter</a>
                 <a href="{{ route('outlet.outlet-create-request') }}" class="btn btn-primary">
                        Create Request
                    </a>
            </div>
         </form> --}}
                <form method="GET" action="{{ route('outlet.outlet-transfer-request') }}"
                    class="search-record-box flex items-center justify-between flex-wrap gap-[15px] mb-[15px]"
                    id="transferRequestFilterForm">
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <input type="date" name="date" value="{{ request('date') }}"
                            class="form-control max-w-[100%] " placeholder="Date" id="customDate"
                            onchange="document.getElementById('transferRequestFilterForm').submit()" />
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse($transferStocks as $transfer)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">
                                            {{ Str::words(ucwords($transfer->senderOutlet->name ?? '-'), 5, '...') }}</td>
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <a href="{{ route('outlet.outlet-view-request', $transfer->id) }}"
                                    ...
                                    <tr>
                                        <td colspan="6" class="px-6 py-3 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                ...
                                                <h3><strong>No requests from outlets found.</strong></h3>
