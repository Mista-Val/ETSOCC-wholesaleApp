...
@section('content')
    ...
    <main class="dashboard-screen-bg relative">
        <section class="dashboard-title-section bg-white border-b border-gry-50">
            <div class="container-fluid">
                <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                    <h1 class="h6 text-gry-800">Transfer to Outlets</h1>
                    <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                        ...
                        <span class="text-gry-300">/</span>
                        <span class="body-14 text-gry-800">Transfer to Outlets</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                {{-- <form method="GET" action="{{ route('warehouse.transferoutlets') }}" class="search-record-box flex items-center justify-between flex-wrap gap-[15px] mb-[15px]">
              ...
              <input type="date" name="date" value="{{ request('date') }}"
                   ...
                   <select name="status" class="form-control">
                    ...
                    <a href="{{ route('warehouse.transferoutlets') }}" class="btn btn-secondary">Clear Filter</a>
                     <a href="{{ route('warehouse.transferoutletscreate') }}" class="btn btn-primary">
                ...
                <form method="GET" action="{{ route('warehouse.transferoutlets') }}"
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full whitespace-nowrap text-sm text-left text-gray-600">
                            <thead class="text-gry-900 semibold body-14-regular uppercase border-b border-[#E9E9E9]">
                                <tr>
                                    <th class="px-6 py-3">Transfer ID</th>
                                    <th class="px-6 py-3">Outlet to Transfer</th>
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse($transferStocks as $transfer)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">{{ ucwords($transfer->outlet->name ?? '-') }}</td>
                                        ...
                                        <td class="px-6 py-3">
                                            ...
                                            @endphp
                                            <span
                                                class="{{ $statusColors[$transfer->status] ?? 'text-gray-500' }} body-14-semibold">
                                                {{ ucfirst($transfer->status) }}
                                            </span>
                                        </td>
                                        <td class="px-6 py-3 text-center">
                                            <a href="{{ route('warehouse.transferoutletsorderdetails', $transfer->id) }}"
                                        ...
                                        <td colspan="6" class="px-6 py-3 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <img src="{{ asset('web/images/transfer.png') }}" alt="stock"
                                                    class="w-16 h-16" />
                                                <h3><strong>No transfer to outlets found.</strong></h3>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
