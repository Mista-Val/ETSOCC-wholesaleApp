...
@section('content')
            ...
            <div class="container-fluid">
                ...
                <div class="search-record-box flex items-center justify-between flex-wrap gap-[15px] mb-[15px]">
                    {{-- <div class="search-box relative max-w-[390px] w-full">
                        <select name="outlet_id" id="outletSelect" class="form-control !pr-[50px] w-full">
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <a href="{{ route('outlet.outlet-levels') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </div>
                ...
                <div class="white-box p-0">
                    <div class="overflow-x-auto">
                        <table class="w-full whitespace-nowrap text-sm text-left text-gray-600" id="stocksTable">
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @include('web.outlet.outlet_levels.partials.stocks_table', [
                                    'stocks' => $stocks,
                                ])
                            </tbody>
                        </table>
                    ...
                    {{-- @if ($stocks->hasPages())
                        <div class="flex justify-end mt-4 border-t border-[#E9E9E9] p-[15px]" id="paginationContainer">
                            {{ $stocks->appends(['outlet_id' => request('outlet_id')])->links() }}
                        </div>
                    @endif --}}
                      @if ($stocks->total() > 0)
    ...
    <script>
        document.getElementById('outletSelect').addEventListener('change', function() {
            ...
            let url = "{{ route('outlet.outlet-levels') }}";
            ...
            fetch(url, {
                ...
                .then(data => {
                    ...
                    if (data.hasPages && data.paginationHtml) {
