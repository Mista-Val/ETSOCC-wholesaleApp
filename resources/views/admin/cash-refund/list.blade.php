<div class="main-content side-content pt-0">
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header d-block">
                <h2 class="main-content-title tx-24 mg-b-5">Cash Refund Requests</h2>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Cash Refund Requests</li>
                </ol>
            </div>
                    ...
                    <div class="card custom-card">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h6 class="main-content-label">Cash Refund Requests</h6>
                            ...
                            <div class="table-responsive" wire:loading.remove wire:target="search">
                                <table class="table table-bordered card-table w-100">
                                    ...
                                    <tbody>
                                        @forelse ($cashRefunds as $item)
                                            <tr>
                                                <td>{{ ($cashRefunds->currentPage() - 1) * $cashRefunds->perPage() + $loop->iteration }}</td>
                                                <td>#{{ $item->sale_id }}</td>
                                                <td>{{ ucwords($item->customer->name) ?? 'N/A' }}</td>
                                                <td>${{ number_format($item->refund_amount, 2) }}</td>
                                                <td>{{ $item->location->name ?? 'N/A' }}</td>
                                                <td>{{ ucwords($item->refund_reason) ?? '-' }}</td>
                                                <td>{{ $item->created_at->format('Y-m-d') }}</td>
                                                <td>
                                                    @if($item->status == 'accepted_by_supervisor')
                                                        <span class="badge badge-pill badge-success">Accepted by supervisor</span>
                                                    @elseif($item->status == 'accepted_by_admin')
                                                        <span class="badge badge-pill badge-success">Accepted by Admin</span>
                                                    @elseif($item->status == 'rejected_by_supervisor')
                                                        <span class="badge badge-pill badge-danger">Rejected by Supervisor</span>
                                                    @elseif($item->status == 'rejected_by_admin')
                                        ...
                                        @empty
                                            @include('admin.components.record-not-found', ['colspan' => 9])
                                        @endforelse
                                    </tbody>
                                </table>

                                {{-- Pagination --}}
                                @if ($cashRefunds->total() > 0)
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <div>Total Records: {{ $cashRefunds->total() }}</div>
                                        {{ $cashRefunds->links('vendor.pagination.custom') }}
                                    </div>
                                @endif
    ...
    function acceptRefund(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to accept this refund request?",
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = `/admin/cash-refund/${id}/accept`; 
    ...
    function rejectRefund(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to reject this refund request?",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                let form = document.createElement('form');
                form.method = 'POST';
                form.action = `/admin/cash-refund/${id}/reject`;
