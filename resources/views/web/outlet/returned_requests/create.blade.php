    ...
    <section class="dashboard-title-section bg-white border-b border-gry-50 bg-white">
        <div class="container-fluid">
            <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                <h1 class="h6 text-gry-800">Create Returned Request</h1>
                <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                    ...
                    <span class="text-gry-300">/</span>
                    <span class="body-14 text-gry-800 bold">Return Request</span>
                    <span class="text-gry-300">/</span>
                    <span class="body-14 text-gry-800">Create Return Request</span>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
        <div class="container-fluid">
            <div class="bg-white white-box">
                <form action="{{ route('outlet.returned-requests-store') }}" method="POST">
                    @csrf
                    
                    <!-- Warehouse Selection -->
                    <div class="form-group">
                        <label>Warehouse to Return</label>
                        <select class="form-control" name="outlet_id">
                            <option value="">Select warehouse</option>
                            @foreach($warhouse as $outlet)
                                ...
                                <select class="form-control" name="supplier_name">
                                    ...
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}"
                                            {{ old('supplier_name') == $user->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="remarks" placeholder="Enter reason">{{ old('remarks') }}</textarea>
                        @error('remarks')
                            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
                        @enderror
                    </div>
