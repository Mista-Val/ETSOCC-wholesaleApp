    ...
    <main class="dashboard-screen-bg relative">
        <section class="dashboard-title-section bg-white border-b border-gry-50">
            <div class="container-fluid">
                <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                    <h1 class="h6 text-gry-800">Outlet Stock Request</h1>
                    <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                        ...
                        <span class="text-gry-300">/</span>
                        <span class="body-14 text-gry-800">Outlet Stock Request</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                ...
                <form method="GET" action="{{ route('warehouse.outletStockRequest') }}"
                    class="search-record-box flex items-center justify-between flex-wrap gap-[15px] mb-[15px]"
                    id="warehouseTransferFilterForm">

                    <div class="search-box relative max-w-[390px] w-full">
                        <input type="text" name="search" placeholder="Search by Outlet Name"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <input type="date" name="date" value="{{ request('date') }}"
                            ...
                            onchange="document.getElementById('warehouseTransferFilterForm').submit()" />
                        <a href="{{ route('warehouse.outletStockRequest') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </form>
                ...
                <div class="white-box p-[0]">
                        ...
                        <table class="w-full whitespace-nowrap text-sm text-left text-gray-600">
                            <thead class="text-gry-900 semibold body-14-regular uppercase border-b border-[#E9E9E9]">
                                <tr>
                                    ...
                                    <th class="px-6 py-3">Outlet Name</th>
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse($requestStocks as $request)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-3">#{{ $request->id }}</td>
                                        <td class="px-6 py-3">{{ ucwords($request->senderOutlet->name ?? '-') }}</td>
                                        <td class="px-6 py-3">{{ $request->created_at->format('Y-m-d') }}</td>
                                        <td class="px-6 py-3 text-center">
                                            {{-- FIX: Changed single '=' (assignment) to double '==' (comparison) --}}
                                            @if ($request->status == 'pending')
                                                <a href="{{ route('warehouse.outlet-stock-request-accept', $request->id) }}"
                                                    class="btn btn-primary mr-2">Approve</a>

                                                {{-- Corrected the route name to align with your controller function --}}
                                                <a href="{{ route('warehouse.outlet-stock-request-reject', $request->id) }}"
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <a href="{{ route('warehouse.outletStockRequestDetails', $request->id) }}"
                                        ...
                                        {{-- <td class="px-6 py-3">
                                            @php
                                                $statusColors = [
                                                    ...
                                                    'dispatched' => 'text-[var(--color-secondary-400)]',
                                                    'completed' => 'text-[var(--color-status-success-700)]',
                                                ];
                                            @endphp
                                            <span
                                                class="{{ $statusColors[$request->status] ?? 'text-gray-500' }} body-14-semibold">
                                                {{ ucfirst($request->status) }}
                                            </span>
                                        </td> --}}
                                        {{-- <td class="px-6 py-3 text-center">
                                            @if ($request->status != 'pending')
                                                <a href="{{ route('warehouse.return-view', $request->id) }}"
                                                    class="p-2 rounded-full bg-pink-100 text-pink-500 hover:bg-pink-200 inline-flex items-center justify-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            @else
                                                <a href="{{ route('warehouse.return-list-status-accept', $request->id) }}"
                                                    class="btn btn-primary mr-2">Approve</a>
                                                <a href="{{ route('warehouse.return-list-status-reject', $request->id) }}"
                                                    class="btn btn-outline btn-pink">
                                                    Reject
                                                </a>
                                            @endif
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-3 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <img src="{{ asset('web/images/transfer.png') }}" alt="stock"
                                                    class="w-16 h-16" />
                                                <h3><strong>No outlet stock requests found.</strong></h3>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                    ...
                    @if ($requestStocks->total() > 0)
                        <div class="border-t border-[#E9E9E9] p-[15px]">
                            <div class="flex items-center justify-between">
                                <div class="text-sm text-gray-600">
                                    Showing {{ $requestStocks->firstItem() }} to {{ $requestStocks->lastItem() }} of
                                    {{ $requestStocks->total() }} results
                                </div>
                                <div>
                                    {{ $requestStocks->withQueryString()->links('vendor.pagination.custom-new') }}
                                </div>
                            </div>
                        </div>
                    @endif
