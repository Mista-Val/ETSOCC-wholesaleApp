    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header d-block">
                <h2 class="main-content-title tx-24 mg-b-5">External Cash Inflows</h2>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item active" aria-current="page">All External Cash Inflows</li>
                </ol>
            </div>
                    ...
                    <div class="card custom-card">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h6 class="main-content-label">External Cash Inflows</h6>
                                <div class="d-flex align-items-center">
                                    ...
                                    <a href="{{ route('admin.external-cash-inflow.create') }}" class="btn ripple btn-main-primary">Add New</a>
                                </div>
                            </div>
                            ...
                            <div class="table-responsive" wire:loading.remove wire:target="search">
                                <table class="table table-bordered card-table w-100">
                                    ...
                                    <tbody>
                                        @forelse ($externalCashInflows as $item)
                                            <tr>
                                                <td>{{ ($externalCashInflows->currentPage() - 1) * $externalCashInflows->perPage() + $loop->iteration }}</td>
                                                ...
                                                {{-- <td>{{ $item->received_from }}</td> --}}
                                                 <td class="px-6 py-3">
                                                @if (is_numeric($item->received_from))
                                                    {{ Str::words(ucwords(\App\Models\Location::find($item->received_from)->name ?? '-'), 5, '...') }}
                                                @else
                                                    {{ Str::words(ucwords($item->received_from ?? '-'), 5, '...') }}
                                                @endif
                                            </td>
                                                {{-- <td>{{ $item->remarks ?? '-' }}</td> --}}
                                                <td class="text-center">
                                                    <div class="btn-toolbar table-actions" role="toolbar">
                                                        <div class="btn-group btn-group-sm" role="group">
                                                            <a href="{{ route('admin.external-cash-inflow.show', $item->id) }}" class="btn view" data-toggle="tooltip" title="View">
                                                                <i class="fa fa-eye"></i>
                                                            </a>
                                ...
                                {{-- Pagination --}}
                                @if ($externalCashInflows->total() > 0)
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <div>Total Records: {{ $externalCashInflows->total() }}</div>
                                        {{ $externalCashInflows->links('vendor.pagination.custom') }}
                                    </div>
                                @endif
