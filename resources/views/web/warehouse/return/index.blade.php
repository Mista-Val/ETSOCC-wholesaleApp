        ...
        <section class="dashboard-title-section bg-white border-b border-gry-50">
            <div class="container-fluid">
                <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                    <h1 class="h6 text-gry-800">Returns</h1>
                    <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                        ...
                        <span class="text-gry-300">/</span>
                        <span class="body-14 text-gry-800">Returns</span>
                    </div>
                </div>
            </div>
        </section>
                ...
                <form method="GET" action="{{ route('warehouse.return-list') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <input type="date" name="date" value="{{ request('date') }}"
                            ...
                            onchange="document.getElementById('warehouseReturnFilterForm').submit()" />
                        ...
                        <select class="form-control" name="status"
                            ...
                            <option value="accepted_by_warehouse"
                                {{ request('status') == 'accepted_by_warehouse' ? 'selected' : '' }}>
                                Pending Supervisor
                            </option>
                            <option value="accepted_by_supervisor"
                                {{ request('status') == 'accepted_by_supervisor' ? 'selected' : '' }}>
                                Pending Admin Approval
                            </option>
                            <option value="accepted_by_admin"
                                {{ request('status') == 'accepted_by_admin' ? 'selected' : '' }}>
                                Accepted & Completed
                            </option>
                            <option value="rejected_by_warehouse"
                                {{ request('status') == 'rejected_by_warehouse' ? 'selected' : '' }}>
                                Rejected by Warehouse
                            </option>
                            <option value="rejected_by_supervisor"
                                {{ request('status') == 'rejected_by_supervisor' ? 'selected' : '' }}>
                                Rejected by Supervisor
                            </option>
                            <option value="rejected_by_admin"
                                {{ request('status') == 'rejected_by_admin' ? 'selected' : '' }}>
                                Rejected by Admin
                            </option>
                        </select>

                        <a href="{{ route('warehouse.return-list') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </form>
                        ...
                        <table class="w-full whitespace-nowrap text-sm text-left text-gray-600">
                            <thead class="text-gry-900 semibold body-14-regular uppercase border-b border-[#E9E9E9]">
                                <tr>
                                    ...
                                    <th class="px-6 py-3">Source Outlet</th>
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse($transferStocks as $transfer)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">{{ Str::words(ucwords($transfer->remark ?? '-'), 5, '...') }}
                                        </td>
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <a href="{{ route('warehouse.return-view', $transfer->id) }}"
                                                ...
                                                @if ($transfer->status == 'pending')
                                                    <a href="{{ route('warehouse.return-list-status-accept', $transfer->id) }}"
                                                    ...
                                                    <a href="{{ route('warehouse.return-list-status-reject', $transfer->id) }}"
                                        ...
                                        <td colspan="6" class="px-6 py-3 text-center text-gray-500">
                                            <div class="flex flex-col items-center justify-center gap-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                                                    ...
                                                    <path d="M10 20L5 15L10 10" />
                                                    <path d="M18 15h-8c-2.209 0-4-1.791-4-4V6" />
                                                    <rect x="14" y="3" width="7" height="7" rx="1" />
                                                </svg>
                                                <h3><strong>No returns found.</strong></h3>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
