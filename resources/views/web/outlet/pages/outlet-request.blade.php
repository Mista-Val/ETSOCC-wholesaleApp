...
@section('content')
    ...
    <main class="dashboard-screen-bg relative">
        <section class="dashboard-title-section bg-white border-b border-gry-50">
            <div class="container-fluid">
                <div class="dashboard-inner flex items-center justify-between gap-[10px] flex-wrap py-[10px]">
                    <h1 class="h6 text-gry-800">Outlet Stock Requests</h1>
                    <div class="breadcrumb flex items-center gap-[10px] flex-wrap">
                        ...
                        <span class="text-gry-300">/</span>
                        <span class="body-14 text-gry-800">Outlet Stock Requests</span>
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                <form method="GET" action="{{ route('outlet.outlet-request') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <input type="date" name="date" value="{{ request('date') }}" class="form-control max-w-[100%]"
                            placeholder="Date" id="customDate" onchange="document.getElementById('filterForm').submit()" />
                        <a href="{{ route('outlet.outlet-request') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </form>
                ...
                <div class="white-box p-0">
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse($stocks as $index => $stock)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-3">#{{ $stock->id }}</td>
                                        <td class="px-6 py-3">{{ ucwords($stock->outlet->name) }}</td>
                                        ...
                                        <td class="px-6 py-3">
                                            ...
                                            @endphp
                                            <span
                                                class="{{ $statusColors[$stock->status] ?? 'text-gray-500' }} body-14-semibold">
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <!-- View Button -->
                                                <a href="{{ route('outlet.outlet-request-show', $stock->id) }}"
                                                ...
                                                <!-- Approve/Reject Buttons - Only show when status is 'created' -->
                                                @if (isset($stock->status) && $stock->status == 'created')
                                                    <a href="{{ route('outlet.outlet-request-status-accept', $stock->id) }}"
                                                        class="btn btn-primary px-3 py-1.5 text-sm">
                                                        Approve
                                                    </a>
                                                    <a href="{{ route('outlet.outlet-request-status-reject', $stock->id) }}"
                                                        class="btn btn-outline btn-pink px-3 py-1.5 text-sm">
                                                        Reject
                                                    </a>
                                                @endif
                                            </div>
                    ...
                    <!-- Pagination Links -->
                    {{-- <div class="flex justify-center mt-4 border-t border-[#E9E9E9] p-[15px]">
