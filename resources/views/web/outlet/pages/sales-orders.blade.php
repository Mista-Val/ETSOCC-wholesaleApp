...
@section('content')
    ...
    {{-- Waybill Invoice Modal for Outlet --}}
            ...
            <div class="modal-content p-6">
                <form method="POST" action="" id="waybill-form">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        {{-- Outlet Name (Read-only from logged-in outlet) --}}
                        <div class="form-group">
                            <label class="block mb-2 text-sm font-medium text-gray-700">
                                Outlet Name <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="outlet_name"
                                value="{{ auth()->guard('outlet')->user()->outlet->name ?? '' }}" readonly
                        ...
                        {{-- Warehouse Name (Dropdown from locations where type='warehouse') --}}
                        {{-- <div class="form-group">
                            <label class="block mb-2 text-sm font-medium text-gray-700">
                ...
                <form method="GET" action="{{ route('outlet.salesOrders') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <input type="date" name="date" value="{{ request('date') }}"
                            ...
                            onchange="document.getElementById('salesOrdersFilterForm').submit()" />

                        <a href="{{ route('outlet.salesOrders') }}" class="btn btn-secondary">Clear Filter</a>

                        <a href="{{ route('outlet.createSales') }}" class="btn btn-primary">
                            Create Sale
                        </a>
                    </div>
                </form>
                            ...
                            <tbody class="divide-y divide-gray-100 body-14-regular text-gry-500">
                                @forelse ($sales as $sale)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">
                                            {{ Str::words(ucwords($sale->customer->name ?? '-'), 5, '...') }}</td>
                                        ...
                                        <td class="px-6 py-3">
                                            <div class="flex items-center gap-[10px]">
                                                {{-- View Details Button --}}
                                                <a href="{{ route('outlet.salesDetails', $sale->id) }}"
                                                ...
                                                {{-- Download Invoice Button (only if completed) --}}
                                                @if ($sale->status == 'completed')
                                                    <a href="{{ route('outlet.salesInvoice', $sale->id) }}"
                                                ...
                                                {{-- Conditional Waybill Button (only if completed) --}}
                                                @if ($sale->status == 'completed')
                                                    @if ($sale->waybill && $sale->waybill->status === 'delivered')
                                                        {{-- Download Waybill Button --}}
                                                        <a href="{{ route('outlet.viewWaybillInvoice', $sale->waybill->id) }}"
                                                ...
                                                @if ($sale->status == 'completed' && $sale->refund_status === 'pending')
                                                    <button
    ...
    <script>
        ...
        function openWaybillModal(saleId, orderId, customerName) {
            ...
            // Set form action URL
            const waybillUrlBase = "{{ route('outlet.generateWaybill', ['sale' => 'PLACEHOLDER']) }}";
        ...
        function openRefundModal(saleId, orderId, customerName) {
            ...
            fetch(`/outlet/sales/${saleId}/get-details`)
                .then(response => {
                    if (!response.ok) {
                ...
                .then(data => {
                    ...
                    // Set form action URL
                    form.action = `/outlet/sales/${saleId}/refund`;
                })
                .catch(error => {
