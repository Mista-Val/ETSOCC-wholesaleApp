    ...
    <div class="container-fluid">
        <div class="inner-body">
            ...
            <div class="page-header d-block">
                <h2 class="main-content-title tx-24 mg-b-5">Cash Remittance Reports</h2>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('admin/dashboard') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('admin.cash-remittance.index') }}">Reports</a></li>
                    <li class="breadcrumb-item active" aria-current="page">All Cash Remittance</li>
                </ol>
            </div>
            {{-- ------------------------------------------------ --}}
                    ...
                    <div class="card custom-card">
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h6 class="main-content-label">Cash Remittance Reports</h6>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="form-group mb-0 mr-3">
                                        <div class="form-group mb-0 table-filters">
                                            ...
                                            <input type="search" class="form-control"
                                                placeholder="Search Receiver/Location/Remark"
                                                wire:model.debounce.300ms="search" />
                                        </div>
                                    </div>

                                    {{-- Role Filter (admin, warehouse, outlet) --}}
                                    <div class="mr-3">
                                        <div class="form-group mb-0 table-filters">
                                            <i class="fa fa-filter mr-2 filter-icon"></i>
                                            <div class="select-down-arrow">
                                                <select class="form-control cp" wire:model="role">
                                                    <option class="text-capitalize" value="">Receiver Role</option>
                                    ...
                                    {{-- Status Filter (pending, created, dispatched, partially accepted) --}}
                                    <div class="mr-3">
                                        <div class="form-group mb-0 table-filters">
                                            ...
                                            <div class="select-down-arrow">
                                                <select class="form-control cp" wire:model="status">
                                                    ...
                                                    {{-- <option class="text-capitalize" value="partially accepted">Partially accepted</option> --}}
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            {{-- Remittance Table --}}
                            <div class="table-responsive" wire:loading.remove wire:target="search, role, status">
                                <table class="table table-bordered card-table w-100">
                                    ...
                                    <tbody>
                                        @forelse ($remittances as $remittance)
                                            <tr>
                                                <td>{{ ($remittances->currentPage() - 1) * $remittances->perPage() + $loop->iteration }}
                                                </td>
                                                <td>{{ $remittance->created_at->format('Y-m-d') }}</td>
                                                <td><span class="badge badge-pill badge-primary-light">$
                                                        {{ $remittance->amount }}</span></td>
                                                <td><span
                                                        class="badge badge-pill badge-info-light">{{ ucfirst($remittance->role) }}</span>
                                                </td>
                                                <td>
                                                    <span class="badge badge-pill badge-warning-light">
                                                        {{ $remittance->location->name ?? 'N/A' }} </span>
                                                </td>
                                                <td><span
                                                        class="badge badge-pill badge-danger-light">{{ $remittance->coustomer->name ?? 'N/A' }}</span>
                                                </td>
                                                <td><span
                                                        class="badge badge-pill badge-{{ $remittance->status === 'created' ? 'success' : ($remittance->status === 'pending' ? 'warning' : 'primary') }}">{{ ucfirst($remittance->status) }}</span>
                                                </td>
                                            </tr>
                                        @empty
                                ...
                                @if ($remittances->total() > 0)
                                    <div class="d-flex justify-content-between align-items-center mt-3">
                                        <div>Total Records : {{ $remittances->total() }}</div>
                                        {{ $remittances->links('vendor.pagination.custom') }}
                                    </div>
                                @endif

                                <div wire:loading.delay wire:target="search, role, status" class="w-100">
