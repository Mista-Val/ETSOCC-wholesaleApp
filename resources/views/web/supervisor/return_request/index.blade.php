...
@section('content')
    ...
    <main class="dashboard-screen-bg relative">
        ...
        <section class="dashboard-content py-[15px] md:py-[30px] flex-1">
            <div class="container-fluid">
                <form method="GET" action="{{ route('supervisor.return-request') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        ...
                        <select class="form-control" name="status"
                            ...
                            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>Pending Warehouse</option>
                            <option value="accepted_by_warehouse" {{ request('status') == 'accepted_by_warehouse' ? 'selected' : '' }}>Pending Supervisor</option>
                            <option value="accepted_by_warehouse_supervisor" {{ request('status') == 'accepted_by_warehouse_supervisor' ? 'selected' : '' }}>Pending Admin Approval</option>
                            <option value="accepted_by_all" {{ request('status') == 'accepted_by_all' ? 'selected' : '' }}>Accepted & Completed</option>
                            <option value="rejected_by_warehouse" {{ request('status') == 'rejected_by_warehouse' ? 'selected' : '' }}>Rejected by Warehouse</option>
                        ...
                        <a href="{{ route('supervisor.return-request') }}" class="btn btn-secondary">Clear Filter</a>
                    </div>
                </form>
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                <!-- Approve button - only for accepted_by_warehouse status -->
                                                @if ($transfer->status == 'accepted_by_warehouse')
                                                    <a href="{{ route('supervisor.return-request-accept', $transfer->id) }}"
                                                        class="p-2 rounded-full bg-green-100 text-green-500 hover:bg-green-200 inline-flex items-center justify-center"
                                                ...
                                                <!-- Reject button - show for all except rejected and accepted_by_all statuses -->
                                                @if (!in_array($transfer->status, [
                                                    ...
                                                    <a href="{{ route('supervisor.return-request-reject', $transfer->id) }}"
                                                        class="p-2 rounded-full bg-red-100 text-red-500 hover:bg-red-200 inline-flex items-center justify-center"
                                                ...
                                                @if (in_array($transfer->status, [
                                                    ...
                                                    'rejected_by_admin',
                                                    'accepted_by_all',
                                                ]) && $transfer->status != 'accepted_by_warehouse')
