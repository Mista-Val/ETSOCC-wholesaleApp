...
@section('content')
    ...
    <div id="add-customer-modal"
        ...
        <div class="modal-box bg-white rounded-xl shadow-lg w-full max-w-md p-0 relative" onclick="event.stopPropagation()">
            ...
            <div class="modal-content p-4">
                <form method="POST" action="{{ route('warehouse.customerStore') }}" id="add-customer-form">
                    @csrf
    ...
    <main class="dashboard-screen-bg relative">
        ...
        <section class="dashboard-content py-4 md:py-8 flex-1">
            <div class="container-fluid">
                ...
                <form method="GET" action="{{ route('warehouse.customerList') }}"
                    ...
                    <div class="date-record-btn w-full md:w-auto flex items-center gap-[15px] flex-wrap md:flex-nowrap">
                        <a href="{{ route('warehouse.customerList') }}" class="btn btn-secondary">Clear Filter</a>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse($customers as $customer)
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-3">
                                            <span class="max-w-xs truncate block" title="{{ $customer->address ?? 'N/A' }}">
    ...
    <script>
        ...
        function openEditModal(customerId, customerName, customerPhone, customerAddress) {
            ...
            const updateUrlBase = "{{ route('warehouse.customerUpdate', ['customer' => 'PLACEHOLDER']) }}";
        ...
        function openDeleteModal(customerId, customerName) {
            ...
            const deleteUrlBase = "{{ route('warehouse.customerDestroy', ['customer' => 'PLACEHOLDER']) }}";
            form.action = deleteUrlBase.replace('PLACEHOLDER', customerId);

            if (modalText) {
                modalText.innerHTML = `Are you sure you want to delete customer <b>${customerName}</b>? This action cannot be undone.`;
            }
        }
